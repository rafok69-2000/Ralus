...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-950">
      {/* ── Sidebar ──────────────────────────────────────────────────────── */}
      <aside className="w-60 shrink-0 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 fixed left-0 top-0 bottom-0 flex flex-col z-10">
        {/* Logo */}
        <div className="px-5 py-5 flex items-center gap-2.5 border-b border-gray-100 dark:border-gray-700">
          ...
          <span className="text-base font-bold text-gray-900 dark:text-gray-100">Ralus</span>
        </div>
              ...
              {projects.map((project) => (
                <li key={project.id}>
                  <button
                    ...
                    className="w-full text-left px-2 py-1.5 rounded-lg text-sm text-gray-700 dark:text-gray-300
                      hover:bg-violet-50 dark:hover:bg-violet-900/30 hover:text-violet-600 dark:hover:text-violet-400 transition flex items-center gap-2"
                  >
                    <span
        ...
        {/* Bottom actions */}
        <div className="px-3 py-3 border-t border-gray-100 dark:border-gray-700 flex flex-col gap-2">
          <button
            onClick={openModal}
            className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm font-medium
              bg-violet-600/10 hover:bg-violet-600/20 text-violet-600 dark:text-violet-400
              hover:text-violet-700 dark:hover:text-violet-300
              border border-violet-600/20 transition-colors"
          >
          ...
          <div className="flex items-center gap-2.5 px-2 py-1.5">
            ...
            <span className="flex-1 text-sm text-gray-700 dark:text-gray-300 truncate">{user?.name}</span>
            <button
      ...
      <main className="ml-60 flex-1 px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-7">
          <div>
            <h2 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Mis proyectos</h2>
            {!loadingProjects && (
        ...
        {/* Grid */}
        {loadingProjects ? (
          <p className="text-gray-500 dark:text-gray-400 text-sm">Cargando proyectos...</p>
        ) : projects.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-center">
            <div className="text-5xl mb-4">📋</div>
            <p className="text-gray-900 dark:text-gray-400 text-lg font-medium">No tienes proyectos aún</p>
            <p className="text-gray-500 dark:text-gray-600 text-sm mt-1">Crea tu primer proyecto para comenzar</p>
            <button
              onClick={openModal}
              className="mt-4 bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors"
            >
              + Nuevo proyecto
            </button>
          </div>
            ...
            {projects.map((project) => (
              <div
                ...
                className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700
                  hover:border-violet-300 dark:hover:border-violet-500 rounded-xl overflow-hidden cursor-pointer
                  transition-all hover:shadow-lg dark:hover:shadow-violet-900/20 hover:-translate-y-0.5 group"
              >
                {/* Color bar */}
                <div className="h-2 rounded-t-xl" style={{ backgroundColor: project.color ?? '#8B5CF6' }} />
                ...
                <div className="p-5">
                  ...
                  {project.description ? (
                    ...
                    <p className="text-sm text-gray-300 dark:text-gray-600 italic">Sin descripción</p>
                  )}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nuevo proyecto">
        <form onSubmit={handleCreate} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="proj-name" className="text-sm font-medium text-gray-700 dark:text-gray-300">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="proj-desc" className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Descripción
            </label>
            <textarea
              id="proj-desc"
